            ...
            <ContextMenu key={waifu?.id}>
              <ContextMenuTrigger>
                <div className="bg-black flex flex-col p-4 w-full h-fit border-white border-3 mb-4">
                  <Image unoptimized src={decodeURI(waifu?.linkGambar)} alt={`Image for ${waifu?.nama}`} width={310} height={310} draggable={false} className="w-full cursor-pointer transition ease-out duration-200 hover:scale-102" onClick={() => window.open(waifu?.linkGambar)} />
