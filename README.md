...
- Second, make a table with atleast this structure `id (int), nama (varchar), linkGambar (varchar), umur (varchar), ultah (varchar), asal (varchar)` and make id as an auto incremented primary key, then put the table's name into the `MYSQL_TABLE` env variable. You can make the sizes as big as you'd like it to be  
