...
- Second, make a `.env` file on the project's root with this format if you will use a database or just empty them all and set the `USE_DATABASE` variable value to false if you don't want to  
```env
...
MYSQL_PORT=<Your database port, default will be 3306>
...
# Note
If you set the `USE_DATABASE` value to false then your waifu won't be saved when you exit out of the web, if you want them to persist then use database instead
